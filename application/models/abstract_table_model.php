...
abstract class abstract_table_model extends CI_Model {
	...
	function like() {
		...
		for($i = 0; $i < $count; $i++) {
			switch($args[$i]) {
				case 'or': $this->db->or_like($args[++$i], $args[$i+1], $this->like_type); break;
				default: $this->db->like($args[$i], $args[$i+1], $this->like_type);
			}
		}
	...
	function not_like() {
		...
		for($i = 0; $i < $count; $i++) {
			switch($args[$i]) {
				case 'or': $this->db->or_not_like($args[++$i], $args[$i+1], $this->like_type); break;
				default: $this->db->not_like($args[$i], $args[$i+1], $this->like_type);
			}
		}
	...
	function having() {
		...
		for($i = 0; $i < $count; $i += 2) {
			switch($args[$i]) {
				case 'or': $this->db->or_having($args[++$i], $args[$i+1]); break;
				default: $this->db->having($args[$i], $args[$i+1]);
			}
		}
